
 <h1><center>Special Growth Combos</center></h1>
***
Below is a list of **Special**-type **Growth Combos** that exist within the game, as well as their passive effects, their **Augments**, and the **Challenges** that unlock those **Augments**.

This list will be sorted by *Primary* **Attribute**. Each heading will be *Primary*, and then each bullet point within that heading will be the associated *Secondary* **Attribute** that induces this **Combo**.

### Ghosting
- **Finesse**:
- **Impact**:
- **Flow**: *Spectral Swipes*
	- Successive **Strikes** grant increasing **Invulnerability** during **Strike Windup**.
	- **Augment**: When **Invulnerability** is at its maximum, become able to **Cancel** a **Strike**'s **Recovery** with another **Strike**.
- **Fortitude**:
- **Resonance**:
### Finesse
- **Ghosting**:
- **Impact**:
- **Flow**: *Verdict*
	- After a **Perfect Parry**, if you **Counter** at the right time, it becomes a **Perfect Counter** instead. 
		- Can *not* be **Parried** or **Dodged**. 
		- It deals 1.25x the damage of a normal **Counter**.
	- **Augment**: Does *double damage* and has *no* **Recovery**.
- **Fortitude**: *Keen Blade*
	- Doubles the **Perfect Parry** window.
	- **Augment**: Triples it.
- **Resonance**:

### Impact
- **Ghosting**:
- **Finesse**:
- **Flow**: *Bloodletter*
	- 10% of **Resilience** damage is dealt as **Health** damage instead.
	- **Augment**: 20%
- **Fortitude**: *Poise Drain*
	- Every 3rd *consecutive* **Perfect Parry**, deal bonus **Resilience** damage, and restore some **Max Resilience**
	- **Augment**: Every **Perfect Parry**.
- **Resonance**:

### Flow
- **Ghosting**:
- **Finesse**: *Relentless Riposte*
	- Successive **Perfect Parries** decrease **Recovery** of **Perfect Parries**, allowing more opportunities to **Counter**.
	- **Augment**: Now also applies to **Dodge**. (Successive **Dodges** decrease **Recovery** of **Perfect Parries**)
- **Impact**:
- **Fortitude**:
- **Resonance**:

### Fortitude
- **Ghosting**:
- **Finesse**:
- **Impact**:
- **Flow**: *Stridebreacher*
	- **Counters** do guaranteed **Resilience** damage.
	- **Augment**: Even when **Parried**.
- **Resonance**: *Soulguard*
	- **Blocking** will consume half the usual **Resilience**; the other half is consumed as **Energy**.
	- **Augment**: All **Resilience** damage is deducted from **Energy** instead, if available.
### Resonance
- **Ghosting**: *Needlethreader*
	- Whenever your **Invulnerability** mitigates damage, your next **Stigmata** use has its **Energy** cost reduced by 30%
	- **Augment**: 60%
- **Finesse**:
- **Impact**:
- **Flow**:
- **Fortitude**:
